...
return (count_chars);
 ...
 if (format[*i] == 'd' || format[*i] == 'i')
        return (print_int(cart, flags, length, field_width, precision));
if (format[*i] == 'b') 
        return (print_binary(va_arg(cart, unsigned int)));
 if (format[*i] == 'u')
        return (print_unsigned(cart, flags, length, field_width, precision));
    if (format[*i] == 'o')
        return (print_octal(cart, flags ,length, field_width, precision));
    if (format[*i] == 'x')
        return (print_hex(cart, 0, flags, length, field_width, precision));
    if (format[*i] == 'X')
        return (print_hex(cart, 1, flags, length, field_width, precision));
    if (format[*i] == 'S')
